...
func Respond(botUrl string, update telegram.Update) {
	...
	if update.Message.Text == "" {
		telegram.SendStck(botUrl, update.Message.Chat.ChatId, "CAACAgIAAxkBAAIBI2PrfKjtI2x-jY1WAs5MjFRBm6JwAAInAAOldjoOspa6vsFKQhkuBA")
		return
	}

	// Проверка на манипуляции с БД
	if strings.ContainsAny(update.Message.Text, "`'%+$") {
		telegram.SendMsg(botUrl, update.Message.Chat.ChatId, "Bad request")
		return
	}
	...
	case "/help", "/start":
		telegram.SendMsg(botUrl, update.Message.Chat.ChatId, "Syntax:\n<b>/search alcoholic=no flavour=spicy</b> - <i>all non-alcoholic spicy drinks</i>\n<b>/search type=promo shortcut=3xT</b> - <i>all promo drinks with 3 Karmotrine</i>\n<b>/search name=piano</b> - <i>\"Piano Man\" and \"Piano Woman\" recieps</i>\n\n You can also use\n	<b>/search ice=yes&price=280&description=champaigne</b>")
		telegram.SendStck(botUrl, update.Message.Chat.ChatId, "CAACAgIAAxkBAAIBOmPrgHU_dc2p5aNX_s2tbo8MytiNAAKDAQAC5y5hCC7gW3lr-iVQLgQ")
	default:
		telegram.SendStck(botUrl, update.Message.Chat.ChatId, "CAACAgIAAxkBAAIBRWPrgSjoO8gZfTKgA2N6vXGpo1fNAAK_AAPnLmEI82NgLSCbuiMuBA")
	}
